...
function SyncContent($idHost, $cmHost, $clientSecret, $clientName, $nugetSource, $userName, $password) {    
    try {
        # Add nuget source & install Sitecore CLI
        Write-Host "Installing Sitecore CLI"

        #dotnet nuget add source $nugetSource --name "Velux jFrog" --username $userName --password $password
...
if (Wait-Url $cmHost.Value) {
    ...
    $nugetSource = Get-ChildItem -Path Env:\NUGET_SOURCE;
    $userName = Get-ChildItem -Path Env:\JFROG_USERNAME;    
    $password = Get-ChildItem -Path Env:\JFROG_PASSWORD;

    SyncContent $idHost.Value $cmHost.Value $clientSecret.Value $clientName.Value $nugetSource.Value $userName.Value $password.Value
